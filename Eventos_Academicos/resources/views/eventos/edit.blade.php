<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('evento.update',$evento) }}" 
                ...
                <div class="grid gap-6 mb-6 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="text" id="enlace_evento" name="enlace_evento"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" value="{{old('enlace_evento', $evento->enlace_evento)}}">
                        @error('enlace')
                            <div class="text-sm text-red-400">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select id="tipo_evento_id" name="tipo_evento_id"
                            ...
                            @foreach ($tipoevento as $tipo)
                                <option value="{{ $tipo->id }}" @selected($tipo->id === $evento->tipo_evento_id)> {{$tipo->nombre_tipoevento }}</option>
                            @endforeach
                        </select>
                        @error('country_id')
                            <div class="text-sm text-red-400">{{ $message }}</div>
